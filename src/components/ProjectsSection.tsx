...
const getProjectStatus = (project: Project): "Shipped" | "Under Dev" =>
...
const ProjectCard = ({
  ...
  return (
    <motion.div
      ...
      className={`group grid grid-cols-1 lg:grid-cols-12 gap-8 sm:gap-10 lg:gap-16 items-start`}
    >
      {/* Image Showcase Section */}
      <div
        className={`lg:col-span-7 relative ${index % 2 === 0 ? "lg:order-1" : "lg:order-2"} lg:sticky lg:top-32`}
      >
      ...
      <div
        className={`lg:col-span-5 flex flex-col justify-center space-y-6 md:space-y-8 ${index % 2 === 0 ? "lg:order-2" : "lg:order-1"}`}
      >
        <div>
          <motion.div
          ...
          >
            <span
              className={`text-[10px] md:text-[11px] font-black uppercase tracking-[0.2em] px-3 py-1 rounded-full border ${
                isShipped
          ...
          <h3 className="text-xl sm:text-2xl md:text-4xl lg:text-5xl font-black tracking-tight leading-[1.15]">
            <span className="text-muted-foreground font-mono text-lg sm:text-xl md:text-3xl">
              {String(index + 1).padStart(2, "0")}
            </span>
              ...
              >
                <div className="space-y-3">
                  ...
                  <p className="text-sm md:text-base text-muted-foreground leading-relaxed border-l-2 border-brand-orange/20 pl-4 italic">
                    {project.problem ||
                      "Solving complex user friction points through architectural excellence and intuitive UI patterns."}
                  </p>
                </div>
                <div className="space-y-3">
                  <h4 className="text-[11px] font-black uppercase tracking-widest text-brand-orange flex items-center gap-2">
                  ...
                  <p className="text-sm md:text-base text-muted-foreground leading-relaxed border-l-2 border-brand-orange/20 pl-4">
                    {project.approach ||
                      "Leveraging modern tech-stack components to create a scalable, performant solution centered around user needs."}
                  </p>
                </div>
                <button
        ...
        <div className="flex flex-wrap gap-2.5">
          {project.technologies.slice(0, 5).map((tech) => (
            <span
              key={tech}
              className="px-4 py-1.5 rounded-lg bg-muted/50 border border-border/50 text-[10px] md:text-[11px] font-black uppercase tracking-wider text-foreground/80 transition-all md:hover:border-brand-orange/40 md:hover:bg-brand-orange/5"
            >
              {tech}
            </span>
          ))}
          ...
          {(project.websiteUrl || project.demoUrl) && (
            ...
            >
              <span className="relative z-10 flex items-center gap-2">
                View Live Project{" "}
                <ExternalLink className="w-4 h-4 text-brand-orange transition-transform group-hover/link:translate-x-1 group-hover/link:-translate-y-1" />
              </span>
...
const ProjectsSection = () => {
  ...
  return (
    ...
    >
      ...
      <div className="container mx-auto px-4 md:px-8 lg:px-10 relative z-10 max-w-[92rem]">
        <motion.div
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true, margin: "-80px" }}
          transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}
          className="w-full rounded-[20px] sm:rounded-[28px] border border-border/60 bg-card shadow-[0_24px_80px_-32px_rgba(15,23,42,0.18)] overflow-hidden"
        >
          <TechStackCarouselBar />

          <div className="px-4 sm:px-6 md:px-10 pt-10 sm:pt-12 md:pt-16 pb-8 sm:pb-10 md:pb-14">
            <motion.div
          ...
          <div className="px-4 sm:px-5 md:px-12 lg:px-14 pb-8 sm:pb-10 md:pb-14 space-y-16 sm:space-y-24 md:space-y-32">
            <AnimatePresence mode="wait">
              <motion.div
                key={currentPage}
            ...
            {totalPages > 1 && (
              <div className="flex flex-col items-stretch sm:items-center justify-between gap-4 sm:gap-6 pt-4 border-t border-border/40">
                <p className="text-xs md:text-sm text-muted-foreground font-semibold text-center sm:text-left">
                  Showing {startIndex + 1}–{Math.min(startIndex + PROJECTS_PER_PAGE, projects.length)} of{" "}
                ...
                <div className="flex flex-wrap items-center justify-center gap-2 w-full sm:w-auto">
                  <Button
                    ...
                    className="rounded-full h-10 px-4 border-border/60 font-bold text-xs uppercase tracking-wider"
                  >
                  ...
                  <div className="flex items-center gap-1.5 px-1">
                    {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                      <button
                        ...
                        className={`h-9 min-w-9 px-2 rounded-full text-xs font-bold transition-colors ${
                          currentPage === page
          ...
          <motion.div
            className="pb-16 md:pb-20 text-center border-t border-border/40 pt-12 md:pt-16 mx-6 md:mx-12 lg:mx-14"
            initial={{ opacity: 0 }}
